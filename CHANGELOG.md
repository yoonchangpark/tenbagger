...
## v6.10 — 요금제 4단계 개편 (2026-09)

### 배경
진입 문턱을 낮추기 위해 Free와 Pro 사이에 5,900원 플랜을 넣는다.

### 변경
- 플랜: **Free ₩0 · 기본 ₩5,900 · Pro ₩9,900 · 플래티넘 ₩19,900** (기존 Premium → 플래티넘으로 개명)
- 등급 경계: 스크리너 무제한은 **기본**부터, AI 정성 분석·투자위원회는 **Pro**부터,
  카카오 알림·백테스트 무제한·API는 **플래티넘**부터.
- 관심종목 한도: free 0 / 기본 10 / Pro 20 / 플래티넘 무제한.
- `TIER_RANK`에 `basic` 추가, `premium` → `platinum` 개명.
  **DB에 남아 있는 `premium` 행은 `LEGACY_TIERS` 매핑으로 `platinum`으로 읽는다**
  (마이그레이션 없이 기존 구독자 보호).
- `pricing.html` 4개 카드·비교표, 결제 완료 페이지 혜택 목록, 결제 승인 허용 등급 갱신.
- 월간 투자 리포트는 **Pro 이상** 혜택 — 기본 플랜에서 제외(플랜 정의·요금제 카드·비교표·결제 완료 화면 4곳).

## v6.9 — 구독 게이트 실제 적용 (2026-09)
