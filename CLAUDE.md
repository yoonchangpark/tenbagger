...
[자동] debug_check.ps1 — 정적 분석 (Edit/Write 즉시)
  ↓
[자동] 디버깅 에이전트 + UX 디자이너 에이전트 — 병렬 동시 실행
  · HTML/JS 변경 시: 두 에이전트를 Agent() 병렬 호출로 동시 실행
  · 발견된 문제는 즉시 수정 후 재검증
  ↓
[자동] 개선 사항 분류
...
| 에이전트 | 자동 실행 조건 | 처리 방식 |
|---------|--------------|----------|
| **디버깅 에이전트** | 항상 (모든 코드 변경 후) | AST 오류·JS 오류·API 불일치·변수 중복·res.ok 미체크 → 즉시 수정 |
| **UX 디자이너 에이전트** | HTML/JS 변경 시 | 빈 상태·에러 메시지·로딩 인디케이터·모바일 레이아웃·탭 흐름·토글 동작 → 즉시 수정 |
| **PM·시니어 개발자** | 큰 개선 필요 시 | 요구사항 정의 → 다음 작업으로 스케줄링 |

#### 디버깅 에이전트 점검 항목 (HTML/JS)

1. `<script>` 태그 열고 닫기 균형
2. `let`/`const` 중복 선언 (같은 스코프)
3. `fetch()` 후 `res.ok` 체크 누락
4. API 응답 필드 불일치 (`data.companies` vs `data.results` 등)
5. `null`/`undefined` 접근 전 guard 없음
6. `encodeURIComponent` 없이 URL 삽입
7. 비동기 함수의 에러 처리(`catch`) 누락

#### UX 디자이너 에이전트 점검 항목 (HTML/JS)

1. API 로딩 중 인디케이터 (스피너·"검색 중…") 존재 여부
2. 데이터 없음/빈 상태 메시지 (단순 사라짐 금지)
3. API 실패 시 에러 메시지 (toast 또는 인라인)
4. 버튼 클릭 후 사용자 피드백 (disabled 상태·toast)
5. 계산 결과가 화면 밖으로 넘어갈 때 `scrollIntoView` 존재 여부
6. 토글(ON/OFF) 시각 상태가 실제 체크박스와 동기화되는지
7. 모바일(≤768px)에서 드롭다운·결과 카드가 잘리지 않는지
8. 엔터키 제출이 필요한 입력 필드에 `onkeydown` 핸들러 존재 여부
