...
## v7.7 — GPT 호출은 막고, 저장된 분석은 그대로 공개 (2026-09)

### 배경

v7.0에서 구독 게이트를 걸었지만, GPT-4o를 부르는 엔드포인트 네 곳이 인증 의존성
없이 열려 있었다. 대시보드는 로그인 없이 둘러볼 수 있으므로 curl이 아니라 **버튼을
눌러서** 도달할 수 있는 경로였다.

- `POST /api/backtest/explain`
- `POST /api/v2/disclosures/analyze`
- `GET  /api/v2/company/{ticker}/quarter-compare` (`refresh=true`)
- `POST /api/v2/portfolio/parse-image`

다만 전부 잠그면 비로그인 방문자가 만나는 첫 화면이 잠금 벽이 된다. 그래서
**돈이 드는 것(새 GPT 호출)만 막고, 이미 만들어 둔 결과는 그대로 보여 준다.**
저장된 분석을 내려주는 데는 토큰 비용이 0원이고, 구독할지 판단할 재료가 된다.

### 서버
- **`allow_cached_or_trial(feature)` 추가** (`core/auth.py`). 비로그인도 통과시키되
  `ctx["require_run"]()`을 부르는 지점에서만 로그인·체험 횟수를 요구한다. 엔드포인트는
  캐시를 먼저 돌려주고, 실제로 GPT를 부르기 직전에 이 함수를 부른다.
  - 투자위원회 `GET /api/v2/committee/{ticker}`
  - 분기 비교 `GET /api/v2/company/{ticker}/quarter-compare`
  - 비로그인에게 돌려주는 캐시 응답에는 `sample: true`를 실어 프런트가 안내를 띄운다.
- **`require_subscription_or_trial` 적용** — 캐시가 없어 항상 GPT를 부르는 경로.
  공시 AI 분석, 포트폴리오 이미지 파싱, 백테스트 해설, 투자위원회 강제 재분석.
- `require_subscription_or_trial`의 등급 판정·차감을 `_trial_state` / `_consume_trial`로
  분리해 두 게이트가 같은 로직을 쓴다. 테이블·모듈은 새로 만들지 않았다.
- 차감은 **응답이 정상일 때만** 일어난다. 실행 중 예외가 나면 제너레이터 teardown을
  건너뛰므로 횟수가 깎이지 않고, 200으로 실패를 알리는 경로는 `consume_trial = False`로
  명시해 둔다. 캐시를 그냥 읽은 요청도 차감하지 않는다.

### 프런트
- 토큰 없이 나가던 호출 세 곳(공시 AI 분석, 분기 비교, 포트폴리오 이미지 파싱)에
  `aiFetch()`를 도입했다. 로그인 상태면 기존 `authFetch`(401 → refresh로 재발급 후
  재시도)를 타고, 비로그인은 토큰 없이 그대로 보낸다. 액세스 토큰이 8시간짜리라
  `authHeaders()`만 붙이면 하루 종일 켜 둔 유료 사용자가 만료 시 "로그인이 필요합니다"를
  보게 되고, 반대로 `authFetch`만 쓰면 비로그인 방문자가 로그인 페이지로 튕긴다.
- `aiLockHtml()` / `sampleHookHtml()` (dashboard), `showSampleNotice()` (index) 추가.
  401은 "로그인하면 3회 무료 체험", 403은 무료(평생 3회 소진)와 기본 플랜(주 3회 소진)을
  서버 안내문으로 구분해 각각 다른 제목·버튼을 보여 준다. 이미 5,900원을 내고 있는
  기본 구독자에게 "구독이 필요합니다"라고 말하지 않는다.
- **투자위원회에서 비로그인 선차단을 제거**했다. 예전에는 `access_token`이 없으면
  호출도 안 하고 잠금 카드를 띄웠지만, 이제 캐시된 위원회 결과를 그대로 보여 주고
  버튼만 "로그인하고 새로 분석"으로 바꾼다.
- 샘플 안내에 **분석일**을 넣었다. 투자 정보는 언제 만든 것인지가 신뢰의 핵심이고,
  분기 비교 캐시는 만료가 없어 몇 달 전 결과가 뜰 수 있다. 하단 라벨도 "캐시:" →
  "분석일:"로 바꿨다.
- 분기 비교에서 재분석이 403이면 **읽고 있던 분석을 지우지 않는다.** 예전 동작대로면
  버튼 한 번에 화면이 잠금 카드로 통째로 바뀌어 읽던 내용을 잃었다.
- 체험 횟수가 0이면 "재분석" 버튼 대신 "다음 주 월요일에 초기화"(기본) 또는
  "구독하고 계속 분석"(무료)을 보여 준다. "0회 남음"은 사람 말이 아니다.

### 함께 고친 것 (같은 화면의 말이 서로 어긋나 있었다)
- AI 정성 분석의 잠금 카드가 "Pro 구독이 필요합니다 / 🔒 Pro 전용"이라고 했지만,
  v7.1 이후 이 기능은 Pro 전용이 아니다(무료 3회·기본 주 3회). 403을 받는 사람은
  "Pro 전용 기능을 건드린 사람"이 아니라 "체험을 다 쓴 사람"이다.
- 403 안내문의 등급명이 소문자로 나가 화면에 "pro 구독이 필요합니다"로 찍히던 문제.
- 위원회 강제 재분석·공시 분석·이미지 파싱·백테스트 해설도 남은 체험 횟수를 응답에
  싣는다. 예전에는 아무 경고 없이 3번 쓰고 4번째에 갑자기 벽을 만났다.
- 백테스트 해설은 OpenAI 키가 없거나 GPT 호출이 실패해도 200을 돌려주는데, 그때도
  체험 1회가 깎이고 있었다. 실패 시 차감하지 않는다.
- 공시 AI 분석이 잠겼을 때 버튼이 "🤖 AI 분석 ▲"(펼침 완료) 라벨로 돌아와, 다시 누르면
  재시도가 아니라 카드가 접히던 문제.
- 로그인 후 돌아올 때 쿼리스트링을 잃던 redirect 세 곳.

### 위원회 캐시 기한 해제
투자위원회 캐시는 24시간짜리라, 어제 누가 안 돌린 종목이면 비로그인 방문자가
결국 잠금 벽을 봤다. 대부분 종목이 여기 해당해서 후킹이 거의 걸리지 않았다.

- **새로 돌릴 수 없는 사람에게는 24시간이 지난 결과라도 돌려준다.** 비로그인과
  체험 소진 양쪽 모두. 저장된 분석을 내려주는 데는 토큰 비용이 0원이다.
  아무 캐시도 없을 때만 401/403이 나가고, `force=true`는 오래된 결과로 때우지 않는다.
- `sample_reason`으로 이유를 구분한다. `login`이면 "로그인하시면 오늘 데이터로
  새로 — 무료 3회", `quota`면 "지금은 새로 돌릴 횟수가 남아 있지 않아 저장된 결과를
  보여 드립니다" + 구독 링크. 로그인한 사람에게 로그인하라고 하지 않는다.
- `_get_cached(ticker, max_age_hours=None)`으로 일반화하면서 기존 `%` 문자열 포맷을
  파라미터 바인딩(`make_interval(hours => :hours)`)으로 바꿨다.

### v1.0 API
`/api/backtest/explain`은 v1 prefix를 쓰지만 v1.0 원본에 없던 신규 엔드포인트다.
`/api/backtest/{ticker}`와 `/api/backtest/market`은 건드리지 않았다.
